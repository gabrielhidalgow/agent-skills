...
**Quick visual reference for design decisions.** An agent skill — plain markdown instructions, not tied
to one vendor.
...
```bash
npx skills add gabrielhidalgow/pinterest-skill
```
...
```bash
/plugin marketplace add gabrielhidalgow/pinterest-skill
```

Then `/plugin install pinterest@pinterest-skill`.

**By hand** — the skill is a directory of plain markdown, so any agent that reads instructions from a file
can use `SKILL.md` directly. The only host-specific line is how to display an image
...
```bash
git clone https://github.com/gabrielhidalgow/pinterest-skill.git ~/code/pinterest-skill
ln -s ~/code/pinterest-skill ~/.claude/skills/pinterest
```
...
| github.com, or another machine | `git -C ~/code/pinterest-skill pull` — then live immediately |
...
```bash
git -C ~/code/pinterest-skill status -sb | head -1     # "## main...origin/main" with no ahead/behind
```
